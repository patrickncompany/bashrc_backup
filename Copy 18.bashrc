...
# alias bsl='box server list' -- replaced with "cms list" (commandbox server list)
function gf() {
    if [ -z "$1" ]; then
        git switch develop;
    else
...
function cw() {
    ...
    echo "- [X] Task One  " >> $cwpath/$cwfile3;
    echo "    - VSCode is trimming double space used for linefeed.  " >> $cwpath/$cwfile3;
    echo "      when editing MD in CMPRO repo.  " >> $cwpath/$cwfile3;
    echo "      MUST edit in AI workspace in NEW editor  " >> $cwpath/$cwfile3;
    echo "- [X] Resolve VSCode whitespace trimming.  " >> $cwpath/$cwfile3;
    echo "    - Worked with Michael for a fix.  " >> $cwpath/$cwfile3;
    echo "- [X] Update scripts for default markdown files.  " >> $cwpath/$cwfile3;
    echo "---  " >> $cwpath/$cwfile3;
    echo "  " >> $cwpath/$cwfile3;
...
function rmlist() {
    ...
    if [ $testmode == "Y" ]; then
        ...
        while read p; do
        echo "rm $p"
        done < /c/Users/KenOwens/Sites/NOTES/$wkai/$wkai-delete.txt
    else
        echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
        echo "!!       -f(force) Used        !!";
        echo "!!                             !!";
