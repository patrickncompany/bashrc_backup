...
alias sites='cd /c/Users/KenOwens/Sites'
alias server='cd /c/CommandBox/server/BBF780B2BA1B5C11D42EAED59A455A34-2023/Adobe-2023.0.08.330668'
...
alias reload="cbs start"
# BURP paths
alias burp='cd /c/Users/BURP'
alias burpapp='cd $HOME/AppData/Local/Programs/BurpSuitePro'
alias burpscripts='cd $HOME/Documents/Selenium\ Scripts/'
alias burpsettings='cd $HOME/AppData/Roaming/BurpSuite'
alias burpprj='cd /c/BURP/Projects'

#need to set env for each, EX: cmpro_home, cxapi_home, cxui_home

# windows terminal pane layouts
    # test layout
    # windows cmd, dos shell only
    # sample :: alias wt1='wt -d "C:\Windows\System32" -p "PowerShell"; sp -V -d "%USERPROFILE%"; nt -p "Ubuntu" --title "wsl" --tabColor "#FF0000"'
    # git-bash
    # sample :: wt split-pane -V; wt move-focus left; wt split-pane -H; wt move-focus right; wt split-pane -H
# base layout
alias wt3='wt sp -H; wt sp -H -p CommandBox; wt move-focus first; wt sp -H'
alias wt4='wt sp -H; wt sp -H -p CommandBox; wt move-focus first; wt sp -V'
...
function vsopen() {
    ...
    openSPEC4="${cArray[4]}";
    # need to add extra spec groupings NOT shown in prompt or list
        # move list from 9 to 99
        # maxis 0-7 due to AIxxxxxxx-files.txt format
        # just know if cmd called on old xxx-files, weirdness
    openSPEC5="${cArray[5]}";
    openSPEC6="${cArray[6]}";
    openSPEC7="${cArray[7]}";
    echo;
    read -p "Open Files (1=all, 2=cfm, 3=sql, 4-7=specs, 99=list, 0=None) : [ 0 ] " openPick;
    if [ -z "$openPick" ]; then
        openPick=0;
    fi;
    # menu item 1-8 = index 0-7
    # consider changing menu number to match index number
    if [ $openPick -eq 1 ]; then
        ...
        eval $openSPEC4;
    elif [ $openPick -eq 6 ]; then
        eval $openSPEC5;
    elif [ $openPick -eq 7 ]; then
        eval $openSPEC6;
    elif [ $openPick -eq 8 ]; then
        eval $openSPEC7;
    elif [ $openPick -eq 99 ]; then
        # base file list on lines 2,3,4,5 not 1 (array 1,2,3)
        # fpass=" --cfm--${cArray[1]} --sql--${cArray[2]} --spec--${cArray[3]} --spec2--${cArray[4]}";
        # need to add condi to only show lines that start with api call of "code"
        fpass=" --all--&&${cArray[0]}&&--cfm--&&${cArray[1]}&&--sql--&&${cArray[2]}&&--spec3--&&${cArray[3]}&&--spec4--&&${cArray[4]}&&--spec5--&&${cArray[5]}&&--spec6--&&${cArray[6]}&&--spec7--&&${cArray[7]}";
        ...
        fpass=$(echo "$fpass" | sed 's/&&/\n/g');  # Replace '&&' with newline
        fpass=$(echo "$fpass" | tr ' ' '\n');  # Replace single spaces with newline
        echo "$fpass";
...
function stash() {
    pida=$(git stash -u -m "quick stash - WIP") &> /dev/null
}
...
function schema() {
    ...
    if [ $ups == "Y" ]; then
        # update schema to current git ai
        pai=$(git rev-parse --abbrev-ref HEAD | xargs);       # parse ai from git
        wkai=${pai:(-10)};                                    # grab ai size chunk
        wkai=$(echo $wkai | tr '[:lower:]' '[:upper:]')       # uppercase
        #echo "Backing up cmpro-alt.xml to $ai workspace.";
        #cp "/c/Users/KenOwens/Sites/cmpro/src/cmpro-alt.xml" "$stpath/cmpro-alt-$timestamp.xml";
        echo "Updating schema."
        sed -i "s/.*<datasource>\(.*\)<\/datasource>.*/            <datasource>$wkai<\/datasource>/" "/c/Users/KenOwens/Sites/cmpro/src/cmpro-alt.xml";
    else
...
function git-clean-branch() {
    ...
    do
        ...
        if [ $uInput == "Y" ]; then
            if [ $testmode == "Y" ];then
                echo;
                echo "TEST MODE >> git branch -D $thisBranch";
                echo;
            else
                echo;
