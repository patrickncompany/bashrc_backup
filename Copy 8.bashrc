...
function vsopen() {
    ...
    openSPEC="${cArray[3]}";
    echo;
    read -p "Open : [ 0 ] (1=all, 2=cfm, 3=sql, 4=spec, 5=list, 0=None) " openPick;
    if [ -z "$openPick" ]; then
        openPick=4;
    fi;
    if [ $openPick -eq 1 ]; then
        ...
        # base file list on lines 2,3,4 not 1 (array 1,2,3)
        fpass="-----${cArray[1]} -----${cArray[2]} -----${cArray[3]}";
        fpass=${fpass//"code"}; # remove code cmd
        fpass=${fpass//"/c/Users/KenOwens/Sites/cmpro/src/"}; # remove path to src
...
function cbs(){
    defaultcba="start";
    if [ -z $1 ]; then
        read -p "Command Box Server [ start | stop ] : " usercba;
        if [ -n $usercba ]; then
            cba=$usercba;
        else
            cba=$defaultcba;
    ...
    defaultcfa="Y";
    read -p "Confirm $cba [ Y | N ] : " usercfa;
    if [ -z $usercfa ]; then
        cfa=$defaultcfa;
    else
        cfa=$usercfa;
